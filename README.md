...
한 걸음 옮기면 몸이 한 겹 줄고, 먹이를 덮으면 느는 **격자 퍼즐.**
Unity 6000.3.22f1 · itch.io 웹 · 시작 2026-08-27

## 🔴 판단이 갈리면

**`docs/project-brief.md`의 §3 설계 원칙과 §4 MVP 범위로 결정한다.**
맥락은 `wiki/`에 있다. 처음이면 `wiki/SCHEMA.md` → `wiki/index.md` 순으로.

## 규칙 (전부다)

1. 조작은 **← →** 둘뿐. 위아래는 중력과 몸집이 정한다
2. 한 걸음 = **크기 −1**. 덮은 먹이 하나당 **+1**. 불은 끄면서 **−1**
3. **중력** — 발밑이 비면 떨어진다. **낙하는 공짜**
4. 앞이 막혔으면 **N−1칸까지 턱을 오른다**. 그것도 한 걸음이다
5. **몸은 가는 쪽으로 흐른다** — 자라면 앞이 부풀고, 줄면 뒤가 딸려온다
6. 크기 0이 되거나 몸이 안 들어가는 걸음은 **불가**. 죽지 않는다 — 못 갈 뿐이다
7. 덩어리가 출구를 덮으면 클리어
...
| 원칙 | 어디에 강제돼 있나 |
|---|---|
| 규칙 정본은 하나 | `tools/engine.js` — C# 사본은 적합성 검사로 묶여 있다 |
| 판 데이터 사본 없음 | `game/Assets/Resources/levels.json` — JS 도구도 이 파일을 직접 읽고 쓴다 |
| 검증 안 된 판은 안 들어감 | `tools/stamp.js`가 0이 아닌 코드로 끝난다 |
| 표기값을 손으로 안 적음 | `stamp.js --write`가 best/sol/back을 박는다 |
| 씬에 손으로 배치한 것 없음 | `Assets/Editor/ProjectBootstrap.cs`가 코드로 만든다 |
| 도트가 안 뭉개짐 | `Assets/Editor/PixelArtImporter.cs`가 임포트 설정을 강제 |
