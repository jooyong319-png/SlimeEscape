...
# ---------------------------------------------------------------- 벽돌 여러 가지
# 🔴 넓은 벽이 한 그림이면 **모눈종이**가 된다. 칸마다 조금씩 다른 걸 섞는다.
#    바탕(경사·명암)은 같게 두고 **무늬만** 바꾼다 — 안 그러면 벽이 울퉁불퉁해 보인다.

def _stone_base():
    """모든 벽돌이 공유하는 바탕. 여기서 명암이 갈리면 벽이 들쭉날쭉해진다."""
    g = blank('3')
    box(g, 0, 0, N - 1, 1, '5')          # 윗면 — 빛을 받는다
    box(g, 0, 2, N - 1, 3, '4')
    box(g, 0, N - 2, N - 1, N - 1, '2')  # 아랫면
    box(g, 0, 0, 0, N - 1, '4')          # 왼쪽이 조금 밝다
    box(g, N - 1, 0, N - 1, N - 1, '2')  # 오른쪽이 그늘
    box(g, 0, N - 1, N - 1, N - 1, '1')  # 칸 사이 이음매
    return g
...
def wall_brick():
    """벽돌 쌓기. 줄눈을 엇갈리게 — 나란히 두면 격자가 다시 보인다."""
    g = _stone_base()
    for y in (10, 21):                       # 가로 줄눈
        box(g, 1, y, N - 2, y, '2')
        box(g, 1, y + 1, N - 2, y + 1, '4')
    for y0, xs in ((4, (11, 23)), (14, (6, 18, 28)), (25, (11, 23))):
        for x in xs:                         # 세로 줄눈 — 층마다 어긋난다
            box(g, x, y0, x, y0 + 5, '2')
    return g


def wall_block():
    """큰 덩어리 하나. 모서리가 조금 깨져 있다."""
    g = _stone_base()
    box(g, 3, 5, N - 4, N - 5, '4')
    box(g, 4, 6, N - 5, N - 6, '3')
    for x, y in [(4, 6), (5, 5), (N - 5, N - 6), (N - 6, N - 5), (3, N - 6), (N - 4, 6)]:
        stamp(g, [(x, y, '2')])              # 깨진 모서리
    return g


def wall_crack():
    """금이 많이 간 돌. 대각선으로 흘러야 금처럼 보인다."""
    g = _stone_base()
    for line in ([(6, 5), (7, 6), (7, 7), (8, 8), (9, 9), (9, 10), (10, 11),
                  (11, 12), (11, 13), (12, 14)],
                 [(24, 16), (23, 17), (23, 18), (22, 19), (22, 20), (21, 21),
                  (20, 22), (20, 23)],
                 [(15, 24), (16, 25), (16, 26), (17, 27)]):
        stamp(g, [(x, y, '2') for x, y in line])
        stamp(g, [(x + 1, y, '4') for x, y in line])
    return g


def wall_rune():
    """
    룬이 새겨진 돌. 🔴 **드물게만** 나와야 한다 —
    흔해지면 무늬가 아니라 배경 소음이 된다.
    """
    g = _stone_base()
    box(g, 9, 8, 22, 23, '2')                # 파낸 자리
    box(g, 10, 9, 21, 22, '3')
    #  손으로 찍은 글자 한 자
    for x, y in [(13, 12), (14, 12), (15, 12), (16, 12), (17, 12), (18, 12),
                 (15, 13), (15, 14), (15, 15), (16, 15), (17, 15),
                 (13, 18), (14, 18), (15, 18), (16, 18), (17, 18), (18, 18),
                 (13, 16), (13, 17)]:
        stamp(g, [(x, y, 'm')])              # 놋쇠빛 — 오래된 새김
    return g
...
def wall_crystal():
    """결정이 박힌 돌. 이것도 드물게."""
    g = _stone_base()
    c = [(15, 11), (16, 11), (14, 12), (15, 12), (16, 12), (17, 12),
         (14, 13), (15, 13), (16, 13), (17, 13), (15, 14), (16, 14),
         (15, 15), (16, 15), (16, 16)]
    stamp(g, [(x, y, 'U') for x, y in c])
    stamp(g, [(x, y, 'W') for x, y in [(15, 12), (16, 13)]])
    stamp(g, [(x, y - 1, 'S') for x, y in c[:6]])   # 박힌 자리 그늘
    return g
...
def _blob(pad, top_flat=False):
    """
    몸 마디 하나. 🔴 **둥근 네모**다. 원이 아니다.

    원 두 개를 나란히 붙이면 맞닿는 곳이 파여서 구슬 목걸이가 된다 (09-02 화면).
    모서리만 깎은 네모면 옆면이 평평해 이웃과 딱 붙고, 이음매가 그 틈만 메우면
    **이어진 관**이 된다. 코드로 그릴 때 쓰던 모양과 같은 이유다.
    """
    g = blank(' ')
    c = (N - 1) / 2.0
    half = c - pad                     # 반쪽 크기
    r = half * 0.55                    # 모서리 깎는 정도
    for y in range(N):
        for x in range(N):
            ...
            if dx > half or dy > half:
                continue
            #  모서리에서만 원으로 깎는다
            ox, oy = max(0.0, dx - (half - r)), max(0.0, dy - (half - r))
            d = (ox * ox + oy * oy) ** 0.5
            if d > r:
                continue
            #  빛은 위에서 온다 — 위가 밝고 아래가 어둡다
            tt = (y - c + half) / (2 * half)
            g[y][x] = 'r' if tt < 0.55 else ('q' if tt < 0.82 else 'p')
            #  🔴 윤곽은 **위아래와 모서리에만.** 좌우 세로면에는 두지 않는다 —
            #     거긴 언제나 이웃이 붙거나 관의 끝이라, 윤곽을 두면 마디가 붙는 자리에
            #     두 겹으로 겹쳐 세로줄이 생긴다 (09-02 화면). 구슬 목걸이가 되는 이유다.
            if d > r - 1.2 or dy > half - 1.2:
                g[y][x] = 'p'
    return g


def body():
    """마디. 손으로 얹는 빛 한 점 — 이거 하나로 **젤리**가 된다."""
    g = _blob(2.5)
...
def link():
    """
    마디 **사이** 이음매. 몸통의 평평한 옆면과 **높이가 같아야** 한다.

    🔴 몸통 한가운데 세로줄의 색을 그대로 옮긴다. 처음엔 밝은 띠를 통째로
       가로질러 놨더니 마디 사이에서 색이 뚝 끊겼다 (09-02 화면).
       빛나는 점은 넣지 않는다 — 마디마다 하나여야지 이음매마다 반복되면
       몸에 점이 줄줄이 박힌 것으로 보인다.
    """
    g = blank(' ')
    c = (N - 1) / 2.0
    half = c - 2.5                     # 몸통과 같은 반쪽 크기
    for y in range(N):
        dy = y - c
        if abs(dy) > half:
            continue
        tt = (dy + half) / (2 * half)
        col = 'r' if tt < 0.55 else ('q' if tt < 0.82 else 'p')
        if abs(dy) > half - 1.2:
            col = 'p'
        box(g, 0, y, N - 1, y, col)
    return g
...
def gem():
    ...
    🔴 레일 위에 겹쳐 놓이므로 **작아야** 한다. 크면 두 겹으로 두꺼워져
       틀이 뭉툭해 보인다 (09-02 화면). 모서리를 찍어주는 정도면 된다.
    """
    g = blank(' ')
    c = (N - 1) / 2.0
    for y in range(N):
